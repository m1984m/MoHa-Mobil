...
{#if $updateAvailable}
  ...
  <div class="fixed left-3 right-3 z-50 surface rounded-2xl border border-base shadow-elev p-4"
       style="bottom: calc(var(--tabbar-space) + 1rem)"
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="t-headline">{$t('Nova različica {version}', { version: APP_VERSION })}</div>
        <div class="t-footnote text-muted">{$t('Kaj je novega:')}</div>
